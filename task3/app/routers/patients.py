...
@router.get("/{patient_id}", response_model=PatientRead, summary="Get a single patient")
...
@router.post("", status_code=status.HTTP_201_CREATED, response_model=PatientRead, summary="Create a patient")
def create_patient(
    data: PatientCreate,
    session: Session = Depends(get_session),
...
@router.patch("/{patient_id}", response_model=PatientRead, summary="Partially update a patient")
def update_patient(
    patient_id: int,
    data: PatientUpdate,
    session: Session = Depends(get_session),
