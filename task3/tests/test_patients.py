...
def test_list_patients_filter_by_condition(client):
    headers = _auth_headers(client, "reader5")
    _create_patient(client, headers, name="Sana", condition="Asthma")
    _create_patient(client, headers, name="Zain", condition="Diabetes")

    res = client.get("/patients", params={"condition": "Asthma"})
    assert res.status_code == 200
    assert all(p["condition"] == "Asthma" for p in res.json())
